...
<body>
	...
	<form action="" method="post">
		...
		<button type="button" id="btn_checkDupIdJson">중복체크J</button>
		<br>
		<p id="checkDupIdMsg"></p>

		사용자 이름 : <input type="text" name="name"><br> 
		사용자 비번 : <input type="password" name="pw"><br> <br>
	...
	<script>
		//중복체크 버튼을 클릭 -> input 태그에 입력한 아이디 값 확인 -> 서버로 ajax요청 (아이디) -> 서버 중복체크 확인
		// -> 중복체크 결과 클라이언트에 응답 -> (클라이언트) 응답 값 확인 -> 화면에표시
			...
			$.ajax({
				...
				success:function(result){
					console.log('ajax success');
 					...
 					//dataType:text -> hson format text(json format 되어있는)
 					...
 					//dataType:json 이면 result에 들어있는것 -> 이미 텍스트를 json 파싱해서 javascript object 변환해서 주입
